...
using ComplexToolCalling.Tools;
...
Configuration configuation = ConfigurationManager.GetConfiguration();
AzureOpenAIClient client = new AzureOpenAIClient(new Uri(configuation.AzureOpenAiEndpoint),
        new System.ClientModel.ApiKeyCredential(configuation.AzureOpenAiKey)
        );

//Get tools via reflection
FileSystemTools target = new();
MethodInfo[] methods = typeof(FileSystemTools).GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
List<AITool> listOfTools = methods.Select(x => AIFunctionFactory.Create(x, target)).Cast<AITool>().ToList();

//Approval Tools
listOfTools.Add(new ApprovalRequiredAIFunction(AIFunctionFactory.Create(RiskyTools.SomethingRisky)));

AIAgent agent = client
    .GetChatClient(configuation.ChatDeploymentName)
    .CreateAIAgent(
    instructions: "You are a helpful assistant and an amazing file system expert. When working with files you need to provide the full path; not just the filename ",
    tools: listOfTools)
    .AsBuilder()
    .Use(FunctionCallMiddleware).Build();
